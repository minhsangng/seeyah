      ...
      <div className='grid grid-cols-2 grid-rows-1 gap-0 w-[90vw] mx-auto h-screen items-center'>
        ...
        <div className='w-3/4 h-5/6 rounded-2xl bg-white/15 border-3 border-white drop-shadow-3xl drop-shadow-white px-4 py-8 justify-self-end overflow-hidden flex flex-col'>
          ...
          <div className='mt-6 grid grid-cols-1 grid-rows-auto gap-2 min-h-0 overflow-y-scroll scrollbar-thin scrollbar-thumb-white'>
            {data.map((item, index) => (
              <div key={item._id}>
                <div className='bg-white/60 flex items-center gap-2 rounded-md flex items-start justify-between gap-2 overflow-hidden border border-amber-300/50'>
                  <div className='pl-12 pr-4 pt-8 pb-2 relative'>
                    <div className='ordinal absolute inset-0 bg-black text-white font-black flex items-center justify-center w-12 h-12 rounded-br-4xl'>{index+1}</div>
                    <p className='line-clamp-1'>{item.name}</p>
                    <button onClikc={`/phim/${item.slug}`} className='border rounded-2xl px-4 py-1 mt-2 bg-amber-200 hover:bg-amber-400 transition ease-linear'>Xem ngay</button>
                  </div>
...
function App() {
  ...
  async function fectchData() {
    try {
      const res = await axios.get('https://ophim1.com/v1/api/danh-sach/phim-moi');
      if (res.status === 200) {
